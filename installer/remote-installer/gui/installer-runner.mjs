...
export function buildConfigFromForm(form = {}) {
  const sshHost = stringValue(form.sshHost)
  const defaultPublicBaseUrl = 'http://127.0.0.1:30080'
  const publicBaseUrl = stringValue(form.publicBaseUrl, defaultPublicBaseUrl).replace(/\/+$/, '')
  const runtimeMode = choice(form.runtimeMode, ['docker-compose', 'k3s'], 'docker-compose')
  const accessMode = choice(form.accessMode, ['local-loopback', 'public-nodeport'], 'local-loopback')
  ...
  const registryProvider = imageMode === 'registry'
    ? choice(form.registryProvider, ['harbor', 'aliyun', 'custom'], 'harbor')
    : 'none'
  const harborHttpPort = numberValue(form.harborHttpPort, 30090)
  const defaultRegistryServer = registryProvider === 'harbor'
    ? `127.0.0.1:${harborHttpPort}`
    : registryProvider === 'aliyun'
      ? 'registry.example.com'
      : 'registry.example.invalid'
  const registryUsernameFallback = registryProvider === 'harbor' ? 'admin' : ''
  const registryPasswordFallback = registryProvider === 'harbor' ? 'Harbor12345' : ''
  ...
  return {
    releaseId: stringValue(form.releaseId),
    ssh,
    git: {
    ...
    runtime: {
      ...
      buildOnRemote: booleanValue(form.buildOnRemote, true),
      deployServices: booleanValue(form.deployServices, true)
    },
    compose: {
    ...
    registry: {
      ...
      providerOptions: ['harbor', 'aliyun', 'custom'],
      server: stringValue(form.registryServer, defaultRegistryServer),
      namespace: stringValue(form.registryNamespace, 'rk-web'),
      username: stringValue(form.registryUsername, registryUsernameFallback),
      password: stringValue(form.registryPassword, registryPasswordFallback),
      dockerConfigJson: stringValue(form.registryDockerConfigJson),
      imagePullSecret: stringValue(form.imagePullSecret, 'rk-registry'),
      installHarbor: registryProvider === 'harbor' ? booleanValue(form.installHarbor, true) : false,
      harborHttpPort,
      harborDataPath: stringValue(form.harborDataPath, '/data/harbor'),
      harborVersion: stringValue(form.harborVersion, '2.14.4'),
      harborDownloadUrls: listValue(form.harborDownloadUrls),
      insecure: registryProvider === 'harbor' ? booleanValue(form.registryInsecure, true) : booleanValue(form.registryInsecure, false)
    }
  }
}
