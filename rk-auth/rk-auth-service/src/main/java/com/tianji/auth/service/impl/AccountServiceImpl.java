...
public class AccountServiceImpl implements IAccountService{
    ...
    public String login(LoginFormDTO loginDTO, boolean isStaff) {
        ...
        detail.setClientType(JwtConstants.CLIENT_TYPE_WEB);
        // 2.2.生成token
    ...
    private String generateToken(LoginUserDTO detail) {
        ...
        storeSessionToken(detail, token);
    ...
    public void logout() {
        ...
        stringRedisTemplate.delete(JwtConstants.SESSION_REDIS_KEY_PREFIX + UserContext.getUser());
        // 删除cookie
        WebUtils.cookieBuilder()
    ...
    private void storeSessionToken(LoginUserDTO detail, String token) {
        Duration ttl = jwtTool.resolveTokenTtl(detail);
        if (ttl == null) {
            ttl = JwtConstants.JWT_TOKEN_TTL;
        }
        stringRedisTemplate.opsForValue().set(
                JwtConstants.SESSION_REDIS_KEY_PREFIX + detail.getUserId(),
                token,
                ttl.getSeconds(),
                TimeUnit.SECONDS
        );
