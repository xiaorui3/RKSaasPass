...
public class AuthUtil {
    ...
    public R<LoginUserDTO> parseToken(String token) {
        ...
        // 6.返回
        R<LoginUserDTO> renewed = renewSlidingSession(token, userDTO);
        if (!renewed.success()) {
            return renewed;
        }
        return R.ok(userDTO);
    ...
    private R<LoginUserDTO> renewSlidingSession(String token, LoginUserDTO userDTO) {
        if (userDTO == null || userDTO.getUserId() == null) {
            return R.error(INVALID_TOKEN_CODE, INVALID_TOKEN);
        }
        String sessionKey = SESSION_REDIS_KEY_PREFIX + userDTO.getUserId();
        String cachedToken = stringRedisTemplate.opsForValue().get(sessionKey);
        if (StringUtils.isBlank(cachedToken)) {
            return R.error(INVALID_TOKEN_CODE, INVALID_TOKEN);
        }
        Duration ttl = resolveSessionTtl(userDTO);
        stringRedisTemplate.expire(sessionKey, ttl);
        String jtiKey = JWT_REDIS_KEY_PREFIX + userDTO.getUserId();
        stringRedisTemplate.expire(jtiKey, ttl);
        return R.ok(userDTO);
    }

    private Duration resolveSessionTtl(LoginUserDTO userDTO) {
        if (userDTO != null
                && userDTO.getClientType() != null
                && CLIENT_TYPE_MOBILE.equalsIgnoreCase(userDTO.getClientType())) {
            return JWT_MOBILE_TOKEN_TTL;
        }
