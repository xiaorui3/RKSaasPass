...
test('teacher should access notice management and see publish button', async ({ page, request }) => {
  const login = await loginApi(request, TENANT1_LIVE_TEACHER)
  await applyLogin(page, login)

  await page.goto(`${BASE_URL}/admin/content/notices`, { waitUntil: 'networkidle' })
  await expect(page.getByText(/鍏憡绠＄悊/).first()).toBeVisible()
  await expect(page.getByRole('button', { name: /鍙戝竷鍏憡/ })).toBeVisible()
})

test('member notifications page should call the real notifications api', async ({ page, request }) => {
