...
test('join invitation should carry referral code', async ({ page, request }) => {
  const seed = Date.now()
  const inviteEmail = `invite_join_${seed}@example.com`
  ...
  const sendRes = await request.post(`${API_BASE}/api/email-center/invitations/send`, {
    ...
    data: {
      ...
      invitationType: 'JOIN',
      subject: `join invite ${seed}`,
      content: 'join invite content',
      referralCode
    }
  })
  ...
  await page.goto(`${BASE_URL}/join?inviteToken=${invitation.inviteToken}`, { waitUntil: 'domcontentloaded' })
