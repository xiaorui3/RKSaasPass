...
test('super admin backup page should call real backup api and render backup records', async ({ page, request }) => {
  const login = await loginApi(request, SUPER_ADMIN)
  const seen = []

  page.on('response', async (response) => {
    if (response.url().includes('/admin/ops/backup/overview')) {
      seen.push({ url: response.url(), status: response.status() })
    }
  })

  await applyLogin(page, login)
  await page.goto(`${BASE_URL}/admin/operation/backup`, { waitUntil: 'networkidle' })

  await expect(page.getByText(/鏁版嵁澶囦唤/).first()).toBeVisible()
  await expect(page.getByRole('button', { name: /绔嬪嵆澶囦唤|鍒涘缓澶囦唤/ }).first()).toBeVisible()
