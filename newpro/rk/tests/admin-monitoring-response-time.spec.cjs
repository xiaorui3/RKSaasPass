...
test('monitoring overview should expose a measured response time for online services', async ({ request }) => {
  ...
  const onlineServices = (body.data?.services || []).filter((item) => item.status === 'online')
  expect(onlineServices.length).toBeGreaterThan(0)
  expect(onlineServices.some((item) => Number(item.responseTime) > 0)).toBeTruthy()
})
